  ...
  return (
    <div>
      ...
      <div className="mb-6 flex items-start gap-3 rounded-xl border border-accent-200 bg-accent-50 px-5 py-4 text-base text-accent-800">
        ...
        <p>
          이 답변은 <strong>실제 AI가 아닌 미리 준비된 시범 응답</strong>입니다. 학생에게 그대로 전달하지 말고 반드시
          교수자가 검토한 뒤 사용해주세요.
        </p>
      </div>

      {showCustomForm && (
        <div className="mb-6 space-y-4 rounded-2xl border border-brand-200 bg-white p-6">
          <h3 className="text-base font-semibold text-brand-900">새 질문 예시 추가</h3>
      ...
      {majors.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[300px_1fr]">
          {/* 입력 영역: 전공과 질문 고르기 */}
          <div>
            <h2 className="mb-4 text-lg font-semibold text-brand-900">1단계 · 전공과 질문 고르기</h2>
            <h3 className="mb-2 text-sm font-semibold text-brand-500">전공</h3>
            <div className="mb-6 flex flex-wrap gap-2 lg:flex-col">
              {majors.map((major) => (
                <button
                  ...
                  className={`rounded-lg px-4 py-2.5 text-left text-base font-medium ${
            ...
            <h3 className="mb-2 text-sm font-semibold text-brand-500">질문</h3>
            <div className="space-y-2">
              {questionsForMajor.map((q) => (
                <button
                  ...
                  className={`flex w-full items-start gap-2 rounded-lg px-3.5 py-3 text-left text-base ${
          ...
          {/* 결과 영역: 선택한 질문의 시범 답변 */}
          <div>
            <h2 className="mb-4 text-lg font-semibold text-brand-900">2단계 · 예시 답변 확인</h2>
            {!selectedQuestion ? (
              <EmptyState
              ...
              <div className="rounded-2xl border border-brand-100 bg-white p-6 sm:p-8">
                <span className="mb-3 inline-block rounded-full bg-brand-100 px-3 py-1 text-sm font-medium text-brand-700">
                  {selectedQuestion.major}
                </span>
                <h3 className="text-xl font-semibold text-brand-950">{selectedQuestion.question}</h3>
                ...
                {!showAnswer ? (
                  ...
                  <div className="mt-5 space-y-6">
                    <div className="rounded-xl bg-brand-50 p-5">
                      ...
                      <p className="text-base leading-relaxed text-brand-900">{selectedQuestion.sampleAnswer}</p>
                    </div>

                    {selectedQuestion.keyConcepts.length > 0 && (
                      <div>
                        <h4 className="mb-2 text-sm font-semibold text-brand-500">핵심 개념</h4>
                        <div className="flex flex-wrap gap-2">
                          {selectedQuestion.keyConcepts.map((c) => (
                            <span key={c} className="rounded-full bg-accent-100 px-3 py-1 text-sm font-medium text-accent-800">
                              {c}
                            </span>
                          ))}
                        </div>
                    ...
                    {selectedQuestion.followUpQuestions.length > 0 && (
                      <div>
                        <h4 className="mb-2 text-sm font-semibold text-brand-500">추가로 물어볼 만한 질문</h4>
                        <ul className="space-y-2 text-base text-brand-700">
